...
# --- আপনার পূর্ণাঙ্গ System Prompt ও Knowledge ---
SYSTEM_PROMPT = """
...
২. যদি কাস্টমার এডমিন বা সরাসরি মানুষের সাথে কথা বলতে চায়, তবে তুমি ঠিক এই লাইনটি বলবে:
...
def handle_all_messages(message):
    ...
    # ১. এডমিন থেকে কাস্টমারকে উত্তর পাঠানো (মেটাডাটা সিস্টেম)
    if user_id == ADMIN_ID and message.reply_to_message:
        try:
            # এডমিনের কাছে পাঠানো মেসেজের একদম শেষ লাইন থেকে কাস্টমার আইডি বের করা
            ...
            bot.reply_to(message, "❌ দুঃখিত, কাস্টমার আইডি পাওয়া যায়নি। অনুগ্রহ করে বটের পাঠানো নোটিফিকেশনে রিপ্লাই দিন।")
            return
    ...
    # ৩. এআই প্রসেসিং এবং চ্যাট ট্রান্সফার
    try:
        ...
        if "ট্রান্সফার করা হচ্ছে" in ai_response:
            ...
            # এডমিনকে তথ্য পাঠানো (এখানে আইডিটি শেষে লিখে দেওয়া হয়েছে যাতে রিপ্লাই কাজ করে)
            bot.send_message(ADMIN_ID, f"🔔 **নতুন সাপোর্ট রিকোয়েস্ট!**\n\nইউজার: @{message.from_user.username}\nবার্তা: {text}\n\n⚠️ উত্তর দিতে এই মেসেজে 'Reply' করুন।\n🆔 ID: {user_id}")
        else:
            bot.reply_to(message, ai_response)
